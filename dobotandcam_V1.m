clear all;
clc;
close all;
rosshutdown;
%% Start Dobot Magician Node
rosinit;

%% Start Dobot ROS
dobot = DobotMagician();

%% Setup webcam
% Create a video input object for the camera
vid = videoinput('linuxvideo', 1);
vid.FramesPerTrigger = 1;
start(vid);

% Create a directory to save the images
outputDirectory = 'captured_images';
if ~exist(outputDirectory, 'dir')
    mkdir(outputDirectory);
end
 
% Initialize a counter for the image filenames
imageCount = 1;

% % Test Motion
% % Publish custom joint target
% joint_target = [0.0,0.4,0.3,0.0];
% dobot.PublishTargetJoint(joint_target);
% 
% %% Publish custom end effector pose
% end_effector_position = [0.2,0,0.1];
% end_effector_rotation = [0,0,0];
% dobot.PublishEndEffectorPose(end_effector_position,end_effector_rotation);

%% Turn on tool
% Open tool
onOff = 1;
openClose = 1;
dobot.PublishToolState(onOff,openClose);
%%
% Close tool
onOff = 1;
openClose = 0;
dobot.PublishToolState(onOff,openClose);

%% Turn off tool
onOff = 0;
openClose = 0;
dobot.PublishToolState(onOff,openClose);

%% Test ESTOP 
%% Send this first
% for i = 0.1:0.05:1.0
%     joint_target = [0.0,i,0.3,0.0];
%     dobot.PublishTargetJoint(joint_target);
%     pause(0.1);
% end

while true
% Position 1
joint_target1 = [0,0,0,0];
dobot.PublishTargetJoint(joint_target1);
pause(2);

    % Capture a single frame/image
    img = getsnapshot(vid);

    % Convert the image to grayscale
    grayImg = rgb2gray(img);

    % Save the grayscale image with an alphabetically ascending filename
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
   
    % Display the captured image
    imshow(grayImg);

    % Increment the image count
    imageCount = imageCount + 1;

pause(1);

% Position 2
joint_target2 = [deg2rad(20),deg2rad(10),deg2rad(-5),0];
dobot.PublishTargetJoint(joint_target2);
pause(2)
    img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 3
joint_target3 = [deg2rad(-20),deg2rad(20),deg2rad(10),0];
dobot.PublishTargetJoint(joint_target3);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 4
joint_target4 = [deg2rad(15),deg2rad(30),deg2rad(20),0];
dobot.PublishTargetJoint(joint_target4);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 5
joint_target5 = [deg2rad(-15),deg2rad(40),deg2rad(30),0];
dobot.PublishTargetJoint(joint_target5);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 6
joint_target6 = [deg2rad(10),deg2rad(50),deg2rad(40),0];
dobot.PublishTargetJoint(joint_target6);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 7
joint_target7 = [deg2rad(-10),deg2rad(60),deg2rad(50),0];
dobot.PublishTargetJoint(joint_target7);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 8
joint_target8 = [deg2rad(5),deg2rad(70),deg2rad(60),0];
dobot.PublishTargetJoint(joint_target8);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 9
joint_target9 = [deg2rad(-5),deg2rad(25),deg2rad(70),0];
dobot.PublishTargetJoint(joint_target9);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);

% Position 10
joint_target10 = [deg2rad(2),deg2rad(15),deg2rad(75),0];
dobot.PublishTargetJoint(joint_target10);
pause(2)
 img = getsnapshot(vid);
    grayImg = rgb2gray(img);
    imageName = fullfile(outputDirectory, sprintf('image_%03d.png', imageCount));
    imwrite(grayImg, imageName);
    imshow(grayImg);
    imageCount = imageCount + 1;
pause(1);
end


%% When the robot is in motion, send this
dobot.EStopRobot();

%% Reinitilise Robot
dobot.InitaliseRobot();

%% Set Rail status to true
dobot.SetRobotOnRail(true);

%% Reinitialise robot. It should perform homing with the linear rail
dobot.InitaliseRobot();

%% Move the rail to the position of 0.1
dobot.MoveRailToPosition(0.1);

%% Set Rail status to false
dobot.SetRobotOnRail(false);

%% Reinitialise robot. It should not perform homing with the linear rail
dobot.InitaliseRobot();

%% Test IO
%% Get current IO status of all io pins on the robot
[ioMux, ioData] = dobot.GetCurrentIOStatus();

%% Set a particular pin a particular IO output
address = 1;
ioMux = 1;
data = 1;
dobot.SetIOData(address,ioMux, data);

%% Set particular pin a particular IO input
address = 1;
ioMux = 3;
data = 0;
dobot.SetIOData(address,ioMux,data);

%% Set a velocity to the conveyor belt
enableConveyor = true;
conveyorVelocity = 15000; % Note: this is ticks per second
dobot.SetConveyorBeltVelocity(enableConveyor,conveyorVelocity);

%% Turn off conveyor belt
enableConveyor = false;
conveyorVelocity = 0; % Note: this is ticks per second
dobot.SetConveyorBeltVelocity(enableConveyor,conveyorVelocity);